...
**Final recommendation: C#/.NET 8 (LTS), three-project solution:**
...
>
> Tasks:
> 1. Create a .NET 8 solution `CreatureGameMaker.sln` with projects: `src/Cgm.Core` (class library, zero UI/graphics dependencies), `src/Cgm.Creator` (Avalonia 11 MVVM app), `src/Cgm.Runtime` (console-launched app using Silk.NET.Windowing + Silk.NET.OpenGL), `tests/Cgm.Core.Tests` (xUnit). Pin all package versions.
