...
export class MoveTester {
   ...
   */
  public static async testMove(move: MoveData): Promise<TestResult> {
    const errors: string[] = [];
    ...
    // Execute
    const result = await MoveEngine.executeMove(attacker, defender, move);
      ...
      switch (effect.type) {
        case "Damage":
          if (
            ...
            const missed = result.events.some((e) =>
              e.message?.includes("missed")
            );
            const immune = result.events.some((e) =>
              e.message?.includes("affect")
            );
            if (!missed && !immune)
              errors.push(`Damage effect didn't reduce HP.`);
          }
          break;
        case "Status":
        ...
        case "Unique":
          if (
            effect.volatileStatus === "LeechSeed" &&
