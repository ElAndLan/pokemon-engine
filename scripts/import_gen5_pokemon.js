...
async function convertToOurFormat(data, id) {
    ...
    for (const move of levelUpMoves) {
        const versionDetail = move.version_group_details.find(v => 
            ...
            (v.version_group.name === 'black-white' || v.version_group.name === 'black-2-white-2')
        );
...
async function importGen5Pokemon() {
    console.log('Starting Gen 5 Pokemon data import...\n');
    ...
    for (let id = 494; id <= 649; id++) {
        try {
            ...
            gen5Pokedex[id.toString()] = converted;
    ...
    const completePokedex = { ...existingPokedex, ...gen5Pokedex };
    ...
    console.log(`\n✓ Complete! Pokedex now has ${Object.keys(sortedPokedex).length} Pokemon (Gen 1-5).`);
}

importGen5Pokemon().catch(console.error);
