...
async function convertToOurFormat(data, id) {
    ...
    for (const move of levelUpMoves) {
        const versionDetail = move.version_group_details.find(v => 
            ...
            (v.version_group.name === 'ruby-sapphire' || v.version_group.name === 'emerald')
        );
...
async function importGen3Pokemon() {
    console.log('Starting Gen 3 Pokemon data import...\n');
    ...
    for (let id = 252; id <= 386; id++) {
        try {
            ...
            gen3Pokedex[id.toString()] = converted;
    ...
    const completePokedex = { ...existingPokedex, ...gen3Pokedex };
    ...
    console.log(`\n✓ Complete! Pokedex now has ${Object.keys(sortedPokedex).length} Pokemon (Gen 1+2+3).`);
}

importGen3Pokemon().catch(console.error);
